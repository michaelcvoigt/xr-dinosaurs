...
class CanvasUI {
  constructor(content, config) {
    ...
    Object.entries(this.config).forEach(([name, value]) => {
      if (typeof(value) === 'object' && name !== 'panelSize' && !(value instanceof WebGLRenderer) && !(value instanceof Scene)) {
        const pos = (value.position !== undefined) ? value.position : {
          x: 0,
          y: 0
        };
    ...
    this.panelSize = (this.config.panelSize !== undefined) ? this.config.panelSize : {
      width: 1,
      height: 1
    }
  ...
  setClip(elm) {
    ...
    if (elm.clipPath !== undefined) {
      ...
      const pos = (elm.position !== undefined) ? elm.position : {
        x: 0,
        y: 0
      };
  ...
  update() {
    ...
    const bgColor = (this.config.body.backgroundColor) ? this.config.body.backgroundColor : 'rgba(0, 0, 0, 0.0)';
    ...
    Object.entries(this.content).forEach(([name, content]) => {
      ...
      if (display !== 'none') {
        const pos = (config.position !== undefined) ? config.position : {
          x: 0,
          y: 0
        };
